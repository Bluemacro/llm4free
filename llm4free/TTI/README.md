# 🖼️ LLM4Free Text-to-Image (TTI) Providers

LLM4Free includes a collection of Text-to-Image providers that follow a common interface inspired by the OpenAI Python client. Each provider exposes an `images.create()` method which returns an `ImageResponse` object containing either image URLs or base64 data.
...
```python
from llm4free.Provider.TTI import BingImageAI
...
```python
from llm4free.Provider.TTI import MagicHourAI
